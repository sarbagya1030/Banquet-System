...
<div class="flex justify-center min-h-screen mt-12">
    <div class="w-3/4">
        <h1 class="text-2xl flex justify-center font-semibold mb-4">Banquet and Parking Capacity</h1>
        <table class="w-full bg-white border">
            <thead>
                <tr>
                    <th class="border p-4">Name</th>
                    <th class="border p-4">Capacity</th>
            ...
            <tbody>
                @foreach ($select as $sc)
                    <tr>
                        <td class="border p-4 text-center">Banquet Capacity</td>
                        <td class="border p-4 text-center">{{ $sc->banquet_capacity }}</td>
                        <td class="flex justify-center border p-4">
                            ...
                            <form action="{{ route('deleteCapacity-banquet', $sc->id) }}" method="POST">
                                @csrf
                                @method('delete')
                                <button type="submit"
                    ...
                    <tr>
                        <td class="border p-4 text-center">Two Wheeler Capacity</td>
                        <td class="border p-4 text-center">{{ $sc->banquet_capacity }}</td>
                        <td class="flex justify-center border p-4">
                            <a href="create-record"
                                class="bg-blue-500 hover:bg-blue-800 text-white px-4 py-2 rounded-lg mr-2">Edit</a>
                    ...
                    <tr>
                        <td class="border p-4 text-center">Four Wheeler Capacity</td>
                        <td class="border p-4 text-center">{{ $sc->banquet_capacity }}</td>
                        <td class="flex justify-center border p-4">
                            <a href="create-record"
                                class="bg-blue-500 hover:bg-blue-800 text-white px-4 py-2 rounded-lg mr-2">Edit</a>
